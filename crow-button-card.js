/**
 * Crow Button Card
 * A liquid-glass entity button for Home Assistant in an iOS 27-style design: pill, tile or square
 * layouts, a glow that flashes while the entity is on, light / dark / auto theming with a
 * glass-opacity slider, your own colours, and your own Home Assistant icon — chosen from HA's own
...
function themeTokens(dark, a) {
  ...
  return dark ? {
    '--bt-ink': '#ffffff', '--bt-ink2': 'rgba(255,255,255,0.72)',
    '--bt-glass1': `rgba(255,255,255,${f(0.10 + a * 0.16)})`,
    '--bt-glass2': `rgba(255,255,255,${f(0.03 + a * 0.08)})`,
    '--bt-edge': 'rgba(255,255,255,0.26)', '--bt-hi': 'rgba(255,255,255,0.42)', '--bt-lo': 'rgba(255,255,255,0.07)',
    '--bt-shadow': '0 14px 36px rgba(0,0,0,0.32)',
    '--bt-chip': 'rgba(255,255,255,0.13)', '--bt-chipedge': 'rgba(255,255,255,0.20)',
    '--bt-track': 'rgba(255,255,255,0.16)', '--bt-tshadow': '0 1px 10px rgba(0,0,0,0.35)',
    '--bt-danger-bg': 'rgba(255,69,58,0.32)', '--bt-danger-ink': '#ffffff',
  } : {
    '--bt-ink': '#1c1c1e', '--bt-ink2': 'rgba(60,60,67,0.72)',
    '--bt-glass1': `rgba(255,255,255,${f(0.50 + a * 0.32)})`,
    '--bt-glass2': `rgba(255,255,255,${f(0.34 + a * 0.30)})`,
    '--bt-edge': 'rgba(255,255,255,0.85)', '--bt-hi': 'rgba(255,255,255,0.95)', '--bt-lo': 'rgba(0,0,0,0.04)',
    '--bt-shadow': '0 10px 30px rgba(28,36,80,0.14), 0 0 0 0.5px rgba(0,0,0,0.05)',
    '--bt-chip': 'rgba(120,120,128,0.12)', '--bt-chipedge': 'rgba(120,120,128,0.10)',
    '--bt-track': 'rgba(120,120,128,0.20)', '--bt-tshadow': 'none',
    '--bt-danger-bg': 'rgba(255,59,48,0.14)', '--bt-danger-ink': '#C4271C',
  };
}
...
// Size scale: 1 = Compact (default), 1.2 = Regular. Every dimension is multiplied by --bt-s.
const S = n => `calc(${n}px * var(--bt-s, 1))`;
...
const STYLES = `
  ...
  /* ── Liquid-glass surface ─────────────────────────────────────────
     ...
     glass opacity / size) and _update() (state colours). */
  ha-card {
    ...
    background: linear-gradient(160deg, var(--bt-glass1), var(--bt-glass2));
    -webkit-backdrop-filter: blur(24px) saturate(170%);
    backdrop-filter: blur(24px) saturate(170%);
    border: 1px solid var(--bt-edge);
    border-radius: ${S(24)};
    box-shadow: var(--bt-rest);
    -webkit-tap-highlight-color: transparent;
    -webkit-user-select: none; user-select: none; -webkit-touch-callout: none;
    cursor: pointer;
    transition: transform .12s ease, border-radius .35s cubic-bezier(.34,1.2,.64,1), box-shadow .35s ease;
  }
  ...
  ha-card::before {
    ...
    background: radial-gradient(80% 55% at 88% -8%, var(--bt-glow, transparent), transparent 72%);
    transition: background .4s;
  }
  /* optional tint while the button is off */
  .bt-tint { position: absolute; inset: 0; z-index: 0; pointer-events: none; background: transparent; transition: background .35s; }
  ha-card.is-off .bt-tint { background: var(--bt-off, transparent); }
  .bt-inner { position: relative; z-index: 1; }

  /* ── Shared bits ─────────────────────────────────────────────── */
  .bt-name {
    font-size: var(--bt-fs); font-weight: 600; letter-spacing: -0.01em; line-height: 1.25;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-width: 0;
    color: var(--bt-name-color, var(--bt-ink));
  }
  .bt-disc {
    position: relative;
    width: ${S(36)}; height: ${S(36)}; border-radius: 50%; flex-shrink: 0; box-sizing: border-box;
    display: flex; align-items: center; justify-content: center;
    background: var(--bt-chip); border: 1px solid var(--bt-chipedge);
    box-shadow: inset 0 1px 0 var(--bt-hi);
    color: var(--bt-text);
    transition: color .3s, background .35s, border-color .35s, box-shadow .35s, transform .12s;
    cursor: pointer;
  }
  .bt-disc::after { content: ''; position: absolute; inset: -6px; }   /* a generous tap area */
  .bt-disc:active { transform: scale(0.92); }
  /* while it is on, the disc fills with the button's colour */
  ha-card.is-on .bt-disc {
    background: linear-gradient(160deg, var(--bt-fill1), var(--bt-fill2));
    border-color: var(--bt-dot);
    box-shadow: inset 0 1px 0 var(--bt-hi), 0 0 12px var(--bt-glowcol);
  }
  .bt-icon {
    --mdc-icon-size: ${S(19)}; width: ${S(19)}; height: ${S(19)}; display: flex; align-items: center; justify-content: center;
    color: var(--bt-icon-color, var(--bt-icon-auto, var(--bt-text)));
  }
  .bt-icon svg { width: 100%; height: 100%; display: block; }
  .bt-mid { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 1px; text-align: var(--bt-align, left); }
  .bt-state {
    font-size: calc(var(--bt-fs) * 0.86); font-weight: 500; line-height: 1.25;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    color: var(--bt-text); font-variant-numeric: tabular-nums;
  }
  .bt-empty { padding: ${S(14)}; font-size: ${S(13)}; color: var(--bt-ink2); }

  /* ── Live Activity pill: one row (height is yours) ───────────── */
  .lay-pill { border-radius: ${S(28)}; }
  .lay-pill .bt-inner {
    display: flex; align-items: center; gap: ${S(10)};
    padding: ${S(8)} ${S(14)} ${S(8)} ${S(10)}; min-height: var(--bt-h); box-sizing: border-box;
  }
  .lay-pill.no-icon .bt-inner { padding-left: ${S(18)}; }
  ...
  .lay-tile .bt-disc { width: ${S(40)}; height: ${S(40)}; }
  .lay-tile .bt-icon { --mdc-icon-size: ${S(21)}; width: ${S(21)}; height: ${S(21)}; }
  ...
  .lay-square .bt-inner {
    ...
    display: flex; flex-direction: column; justify-content: space-between; align-items: var(--bt-just, flex-start);
  }
  .lay-square .bt-disc { width: ${S(40)}; height: ${S(40)}; }
  .lay-square .bt-icon { --mdc-icon-size: ${S(21)}; width: ${S(21)}; height: ${S(21)}; }
  .lay-square .bt-mid { flex: 0 0 auto; width: 100%; }

  /* ── Motion ──────────────────────────────────────────────────────
     Flash pulses the glow while the button is on; its speed is the time between the
     brightest and the dimmest moment.
     subtle / full : flashing as configured (full also lets the icon breathe)
     off           : nothing moves        system : as subtle, but still if "Reduce Motion" is on */
  @keyframes bt-flash { from { opacity: 0.25; } to { opacity: 1; } }
  @keyframes bt-glow {
    from { box-shadow: var(--bt-rest), 0 0 0 0 var(--bt-glowcol0); }
    to   { box-shadow: var(--bt-rest), 0 0 18px 3px var(--bt-glowcol); }
  }
  @keyframes bt-disc {
    from { box-shadow: inset 0 1px 0 var(--bt-hi), 0 0 0 0 var(--bt-glowcol0); }
    to   { box-shadow: inset 0 1px 0 var(--bt-hi), 0 0 14px 3px var(--bt-glowcol); }
  }
  @keyframes bt-warm { 0%,100% { transform: scale(1); } 50% { transform: scale(1.12); } }
  @keyframes bt-spin { to { transform: rotate(360deg); } }
  ha-card.is-flashing::before { animation: bt-flash var(--bt-flash, 600ms) ease-in-out infinite alternate; }
  ha-card.is-flashing { animation: bt-glow var(--bt-flash, 600ms) ease-in-out infinite alternate; }
  ha-card.is-flashing .bt-disc { animation: bt-disc var(--bt-flash, 600ms) ease-in-out infinite alternate; }
  ha-card.anim-full.is-on .bt-icon { animation: bt-warm 2.6s ease-in-out infinite; }
  ha-card.spin-icon.is-on .bt-icon { animation: bt-spin var(--bt-spin, 2s) linear infinite; }
  ha-card.anim-full.spin-icon.is-on .bt-icon { animation: bt-warm 2.6s ease-in-out infinite, bt-spin var(--bt-spin, 2s) linear infinite; }
  ha-card.anim-off, ha-card.anim-off *, ha-card.anim-off::before { animation: none !important; }
...
const DEFAULT_ACTIVE = '#FF3B30';    // the button colour while it is on

const COLOR_PRESETS = [
  { id: 'ruby',     name: 'Ruby',     colors: { active_color: '#FF3B30' } },
  { id: 'amber',    name: 'Amber',    colors: { active_color: '#FF9F0A' } },
  { id: 'mint',     name: 'Mint',     colors: { active_color: '#30D158' } },
  { id: 'ocean',    name: 'Ocean',    colors: { active_color: '#0A84FF' } },
  { id: 'berry',    name: 'Berry',    colors: { active_color: '#BF5AF2' } },
  { id: 'graphite', name: 'Graphite', colors: { active_color: '#98989F' } },
];
// key, label, default, description, optional?
const COLOR_ROWS = [
  ['active_color',   'Active',   DEFAULT_ACTIVE, 'The button colour while it is on', false],
  ['inactive_color', 'Off tint', '#8E8E93',      'Optional — tints the button while it is off (otherwise clear glass)', true],
...
// Services that "toggle" each kind of entity
function toggleService(domain, isOn, state) {
  switch (domain) {
    case 'vacuum':      return ['vacuum', isOn ? 'stop' : 'start'];
    case 'lock':        return ['lock', state === 'locked' ? 'unlock' : 'lock'];
    ...
    default:            return [domain, isOn ? 'turn_off' : 'turn_on'];
  }
}
...
class CrowButtonCard extends HTMLElement {
  constructor() {
    super();
  ...
  static getConfigElement() { return document.createElement('crow-button-card-editor'); }

  static getStubConfig(hass, entities) {
    const pick = (entities || []).find(e => /^(switch|light|input_boolean|script|scene|vacuum)\./.test(e)) || '';
  ...
  static get DEFAULTS() {
    return {
      ...
      show_icon: true, icon: '', use_dynamic_icon: false, spin_icon: false, show_state: true,
      layout: 'pill', text_align: 'left', font_size: '14px', card_height: '56px',
      flash_enabled: true, flash_speed: 600,
      graph_hours: 3,
  ...
  setConfig(config) {
    this._config = { ...CrowButtonCard.DEFAULTS, ...(config || {}) };
    if (this._built) {
  ...
  // The entity the button reads its state from (defaults to the one it controls)
  _stateId() { return this._config?.state_entity || this._config?.entity || ''; }
  ...
  _applyTheme() {
    ...
    this.style.setProperty('--bt-s', String(scale));
    this.style.setProperty('--bt-rest', `inset 0 1px 0 var(--bt-hi), inset 0 -1px 0 var(--bt-lo), var(--bt-shadow)`);
    this.style.setProperty('--bt-fs', `calc(${px(cfg.font_size, 14)}px * var(--bt-s, 1))`);
    this.style.setProperty('--bt-h', `calc(${px(cfg.card_height, 56)}px * var(--bt-s, 1))`);
    // text alignment → how the row / column lines its content up
    const just = align === 'left' ? 'flex-start' : align === 'right' ? 'flex-end' : 'center';
    this.style.setProperty('--bt-align', align);
    this.style.setProperty('--bt-just', just);
  ...
  _build() {
    ...
    const disc = showIcon ? `<div class="bt-disc"><${this._iconTag} class="bt-icon" id="bt-icon"></${this._iconTag}></div>` : '';
    const text = `<div class="bt-mid"><span class="bt-name" id="bt-name"></span><span class="bt-state" id="bt-state"></span></div>`;
    this.shadowRoot.innerHTML = `<style>${STYLES}</style><ha-card id="bt-card" class="lay-${layout}${showIcon ? '' : ' no-icon'}"><div class="bt-tint"></div><div class="bt-inner">${disc}${text}</div></ha-card>`;

    const card = this.shadowRoot.getElementById('bt-card');
    card.addEventListener('click', () => this._onCardTap());
    const discEl = card.querySelector('.bt-disc');
  ...
  _resolve() {
    ...
    const stateObj = stateId ? hass.states[stateId] : null;
    const actionObj = cfg.entity ? hass.states[cfg.entity] : null;
    if (!stateObj) return { stateObj: null };
    const raw = String(stateObj.state);
    const domain = stateId.split('.')[0];
    const attrs = stateObj.attributes || {};
    const offline = raw === 'unavailable' || raw === 'unknown';
    const num = (!offline && raw.trim() !== '' && isFinite(raw)) ? parseFloat(raw) : null;
    const unit = offline ? '' : (attrs.unit_of_measurement || '');

    // which states count as "on": your own list, otherwise the usual active states
    const custom = Array.isArray(cfg.active_state) && cfg.active_state.length ? cfg.active_state.map(String) : null;
    const isOn = !offline && (custom ? custom.includes(raw) : ACTIVE_STATES.includes(raw));

    let text;
    if (offline) text = 'Offline';
    else if (['button', 'input_button', 'scene'].includes(domain)) text = '';
    else if (domain === 'lock') text = raw === 'locked' ? 'Locked' : raw === 'unlocked' ? 'Unlocked' : fmtString(raw);
    else if (domain === 'binary_sensor') { const l = BIN_LABELS[attrs.device_class] || ['On', 'Off']; text = raw === 'on' ? l[0] : raw === 'off' ? l[1] : fmtString(raw); }
    else if (num !== null) text = fmtNumber(num);
    else text = fmtString(raw);

    const kind = offline ? 'offline' : isOn ? 'active' : 'inactive';
    const name = cfg.name || actionObj?.attributes?.friendly_name || attrs.friendly_name || fmtString(cfg.entity || stateId, true);
    // the details sheet needs something to show even for entities whose state is only a time stamp
    const stamp = timeAgo(stateObj.last_changed || stateObj.last_updated).toLowerCase();
    const heroText = text || (domain === 'scene' ? `Activated ${stamp}` : `Pressed ${stamp}`);
    return { stateObj, raw, domain, attrs, offline, num, text, heroText, unit, kind, isOn, name, hasRange: false };
  }

  _palette(r) {
    const dark = this._dark;
    const base = isHex(this._config.active_color) ? this._config.active_color.trim() : DEFAULT_ACTIVE;
    if (r.kind === 'active') return { pal: tuneColor(base, dark), colored: true };
    return { pal: NEUTRAL_COLORS[dark ? 'dark' : 'light'], colored: false };
  }

  _syncIcon(stateObj) {
    const el = this.shadowRoot.getElementById('bt-icon');
    if (!el) return;
  ...
  _update() {
    ...
    if (!r.stateObj) {
      const nameEl = $('bt-name'), stEl = $('bt-state');
      if (nameEl) nameEl.textContent = cfg.entity ? 'Entity not found' : 'Choose an entity';
      if (stEl) { stEl.textContent = cfg.entity || ''; stEl.hidden = !cfg.entity; }
      card.classList.remove('is-on', 'is-flashing');
      return;
    }
    ...
    card.style.setProperty('--bt-c1', pal.c1);
    card.style.setProperty('--bt-c2', pal.c2);
    card.style.setProperty('--bt-dot', pal.dot);
    card.style.setProperty('--bt-text', pal.text);
    card.style.setProperty('--bt-glow', colored ? hexA(pal.dot, dark ? 0.32 : 0.24) : 'transparent');
    if (colored) {
      const ft = fillTint(pal, dark);
      card.style.setProperty('--bt-fill1', hexA(pal.c1, ft.a1));
      card.style.setProperty('--bt-fill2', hexA(pal.c2, ft.a2));
      card.style.setProperty('--bt-glowcol', hexA(pal.dot, dark ? 0.55 : 0.42));
      card.style.setProperty('--bt-glowcol0', hexA(pal.dot, 0));
    }
    card.style.setProperty('--bt-icon-auto', on ? 'var(--bt-ink)' : 'var(--bt-text)');   // ink on the filled disc, the state's own tone when off
    // off tint (optional) and text / icon colour overrides — used exactly as chosen
    const off = isHex(cfg.inactive_color) ? hexA(cfg.inactive_color.trim(), dark ? 0.34 : 0.26) : 'transparent';
    card.style.setProperty('--bt-off', off);
    isHex(cfg.name_color) ? card.style.setProperty('--bt-name-color', cfg.name_color.trim()) : card.style.removeProperty('--bt-name-color');
    isHex(cfg.icon_color) ? card.style.setProperty('--bt-icon-color', cfg.icon_color.trim()) : card.style.removeProperty('--bt-icon-color');
    ...
    const speed = Math.min(3000, Math.max(100, parseInt(cfg.flash_speed, 10) || 600));
    card.style.setProperty('--bt-flash', `${speed}ms`);   // time between the brightest and the dimmest moment
    card.classList.toggle('is-flashing', on && cfg.flash_enabled !== false && anim !== 'off');
    card.classList.toggle('spin-icon', !!cfg.spin_icon);   // anim-off's blanket `animation: none !important` already covers this

    this._syncIcon(r.stateObj);
    const nameEl = $('bt-name'), stEl = $('bt-state');
    if (nameEl) nameEl.textContent = r.name;
    if (stEl) { stEl.textContent = r.text ? `${r.text}${r.unit ? ' ' + r.unit : ''}` : ''; stEl.hidden = cfg.show_state === false || !r.text; }
  }

  // ── Actions ─────────────────────────────────────────────────────
  // tap_action / hold_action: { action: 'toggle' | 'none' | 'perform-action', perform_action: 'domain.service', data, target }
  _actionOf(key) {
    const t = this._config?.[key];
    return (t && typeof t === 'object') ? t : (typeof t === 'string' ? { action: t } : null);
  }
  _tapAction() { return this._actionOf('tap_action'); }
  _holdAction() { const h = this._actionOf('hold_action'); return h && h.action !== 'none' ? h : null; }

  _runAction(a) {
    if (!this._hass || !this._config?.entity) return;
    if (a && a.action === 'none') return;
    const svc = a && a.action !== 'toggle' ? (a.perform_action || a.service || null) : null;   // anything else falls back to the normal toggle
    if (svc && svc.includes('.')) {
      const [domain, name] = svc.split('.');
      this._hass.callService(domain, name, a.data || a.service_data || {}, a.target || {});
      return;
    }
    this._toggleEntity();
  }

  // The normal action: switch the entity the way that kind of entity is switched
  _toggleEntity() {
    const id = this._config?.entity, hass = this._hass;
    if (!id || !hass) return;
    const domain = id.split('.')[0], so = hass.states[id];
    if (!so) return;
    const isOn = this._r?.stateObj && this._stateId() === id ? this._r.isOn : (so.state === 'on');
    const [d, s] = toggleService(domain, isOn, so.state);
    hass.callService(d, s, { entity_id: id });
  }

  // What the button will do, in words (for the sheets)
  _actionLabel() {
    const id = this._config?.entity || '', domain = id.split('.')[0], on = !!this._r?.isOn;
    if (domain === 'vacuum') return on ? 'Stop' : 'Start';
    if (domain === 'lock') return this._hass?.states[id]?.state === 'locked' ? 'Unlock' : 'Lock';
    if (['scene', 'script', 'button', 'input_button'].includes(domain)) return 'Run';
    if (domain === 'cover') return 'Open / close';
    return on ? 'Turn off' : 'Turn on';
  }
  _canAct() { return !!this._config?.entity && this._tapAction()?.action !== 'none'; }

  _hasIcon() { return this._config?.show_icon !== false; }

  // Tapping the icon does the button's action (toggle by default)…
  _onIconTap(e) {
    e.stopPropagation();
    if (this._lpFired) { this._lpFired = false; return; }
    if (!this._config?.entity) return;
    this._runAction(this._tapAction());
  }

  // …tapping anywhere else on the card opens the details sheet (value, history, info).
  // A text-only button (no icon) has nothing else to tap, so the whole button does the action.
  _onCardTap() {
    if (this._lpFired) { this._lpFired = false; return; }
    if (!this._r?.stateObj) return;
    if (this._hasIcon()) this._openPopup();
    else if (this._config?.entity) this._runAction(this._tapAction());
  }

  // Long-press: the AI actions sheet when AI is on; otherwise your hold_action, if you set one;
  // a text-only button opens the details sheet.
  _onLongPress() {
    if (!this._r?.stateObj) return;
    if (this._aiEnabled()) this._openActionsSheet();
    else if (this._holdAction()) this._runAction(this._holdAction());
  ...
  _popupVars() {
    return this._dark
      ? '--bt-ink:#fff;--bt-ink2:rgba(255,255,255,0.68);--bt-line:rgba(255,255,255,0.12);--bt-chip:rgba(255,255,255,0.10);--bt-seg-on:rgba(255,255,255,0.22);--bt-track:rgba(255,255,255,0.16);' +
        '--bt-sheet:linear-gradient(160deg,rgba(70,70,80,0.90),rgba(30,30,36,0.95));--bt-sheet-edge:rgba(255,255,255,0.22);'
      : '--bt-ink:#1c1c1e;--bt-ink2:rgba(60,60,67,0.68);--bt-line:rgba(60,60,67,0.14);--bt-chip:rgba(120,120,128,0.12);--bt-seg-on:#ffffff;--bt-track:rgba(120,120,128,0.20);' +
        '--bt-sheet:linear-gradient(160deg,rgba(255,255,255,0.94),rgba(244,244,250,0.96));--bt-sheet-edge:rgba(255,255,255,0.9);';
  }
  ...
  _createPopupBase(titleText) {
    ...
    style.textContent = `
      ...
      @keyframes btSheetUp { from{transform:translateY(40px);opacity:0} to{transform:none;opacity:1} }
      @media (min-width:700px) { .bt-overlay { align-items:center !important; } }
      .bt-popup {
        background:var(--bt-sheet); border:1px solid var(--bt-sheet-edge); border-radius:34px;
        ...
        font-family:ui-rounded,'SF Pro Rounded',-apple-system,BlinkMacSystemFont,system-ui,'Segoe UI',sans-serif;
        color:var(--bt-ink); animation:btSheetUp 0.38s cubic-bezier(0.32,1.1,0.5,1);
      }
      .bt-close-btn { background:var(--bt-chip);border:none;border-radius:50%;width:32px;height:32px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--bt-ink2);padding:0;flex-shrink:0;font-family:inherit; }
      .bt-info-row { display:flex;align-items:flex-start;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--bt-line); }
      .bt-info-row:last-child { border-bottom:none; }
      .bt-info-label { font-size:13px;color:var(--bt-ink2);font-weight:500;flex-shrink:0;padding-right:12px; }
      .bt-info-value { font-size:13px;font-weight:600;color:var(--bt-ink);text-align:right;word-break:break-all; }
      .bt-seg { display:flex;background:var(--bt-chip);border-radius:12px;padding:3px;gap:2px;margin-bottom:12px; }
      .bt-seg-btn { flex:1;text-align:center;padding:7px 4px;font-size:12px;font-weight:600;border-radius:9px;cursor:pointer;color:var(--bt-ink2);border:none;background:none;transition:all .2s;font-family:inherit;touch-action:manipulation; }
      .bt-seg-btn.active { background:var(--bt-seg-on);color:var(--bt-ink);box-shadow:0 1px 4px rgba(0,0,0,0.25); }
      .bt-graph { position:relative;height:150px;margin-bottom:14px;touch-action:pan-y; }
      .bt-graph-msg { display:flex;align-items:center;justify-content:center;height:100%;color:var(--bt-ink2);font-size:12px; }
      .bt-btn-row { display:flex;gap:10px;margin-top:14px; }
      .bt-btn { flex:1;border:none;border-radius:16px;height:46px;padding:0 16px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit;background:var(--bt-chip);color:var(--bt-ink);transition:opacity .15s,transform .1s; }
      .bt-btn:active { transform:scale(0.98); }
      .bt-tip { position:absolute;top:0;pointer-events:none;padding:5px 9px;border-radius:10px;background:var(--bt-sheet-edge);background:var(--bt-chip);border:1px solid var(--bt-line);font-size:12px;font-weight:600;white-space:nowrap;-webkit-backdrop-filter:blur(12px);backdrop-filter:blur(12px);display:none;color:var(--bt-ink); }
      .bt-tip small { display:block;font-weight:500;color:var(--bt-ink2);font-size:10px; }
      .bt-sec { margin-bottom:16px; }
      .bt-sec[hidden] { display:none; }
      .bt-sec-label { font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--bt-ink2);margin-bottom:6px; }
      .bt-local { font-size:14px;color:var(--bt-ink2);line-height:1.4; }
      .bt-ai-text { font-size:17px;line-height:1.4;color:var(--bt-ink);font-weight:500; }
      .bt-skel { height:14px;border-radius:7px;margin:9px 0;background:linear-gradient(90deg,var(--bt-chip) 25%,var(--bt-line) 50%,var(--bt-chip) 75%);background-size:200% 100%;animation:btShimmer 1.2s linear infinite; }
      @keyframes btShimmer { from{background-position:200% 0} to{background-position:-200% 0} }
      .bt-chips-q { display:flex;flex-wrap:wrap;gap:8px; }
      .bt-q { border:1px solid var(--bt-line);background:var(--bt-chip);color:var(--bt-ink);border-radius:999px;padding:10px 14px;font:inherit;font-size:14px;font-weight:600;cursor:pointer;text-align:left; }
      .bt-q:active { transform:scale(0.98); }
      .bt-answer { margin-top:12px;padding:12px 14px;border-radius:16px;background:var(--bt-chip); }
      .bt-q-title { font-size:12px;font-weight:700;color:var(--bt-ink2);margin-bottom:6px; }
      .bt-ans-body { font-size:15px;line-height:1.45;color:var(--bt-ink); }
      .bt-foot { margin-top:14px;font-size:11px;line-height:1.45;color:var(--bt-ink2); }
      .bt-rows { display:flex;flex-direction:column;border-radius:18px;overflow:hidden;background:var(--bt-chip); }
      .bt-row { display:flex;align-items:center;gap:14px;width:100%;box-sizing:border-box;padding:15px 16px;background:none;border:none;border-top:1px solid var(--bt-line);color:var(--bt-ink);font:inherit;font-size:17px;font-weight:500;text-align:left;cursor:pointer; }
      .bt-row:first-child { border-top:none; }
      .bt-row:active { background:var(--bt-line); }
      .bt-row svg { width:22px;height:22px;flex-shrink:0;color:var(--bt-ink2); }
      .bt-row.is-danger, .bt-row.is-danger svg { color:#FF453A; }
      .bt-stats { display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-bottom:14px; }
      .bt-stat { border-radius:16px;background:var(--bt-chip);padding:12px 14px; }
      .bt-stat b { display:block;font-size:21px;font-weight:700;letter-spacing:-0.02em; }
      .bt-stat span { font-size:12px;color:var(--bt-ink2); }
      .bt-ask-row { display:flex;gap:8px;margin-top:14px; }
      .bt-ask-input { flex:1;min-width:0;box-sizing:border-box;height:44px;padding:0 14px;border-radius:22px;border:1px solid var(--bt-line);background:var(--bt-chip);color:var(--bt-ink);font:inherit;font-size:16px; }
      .bt-ask-input:focus { outline:none;border-color:#0A84FF; }
      .bt-send { width:44px;height:44px;flex-shrink:0;border-radius:50%;border:none;background:#0A84FF;color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer; }
      .bt-send svg { width:20px;height:20px; }
      .bt-tl-head { font-size:13px;color:var(--bt-ink2);margin-bottom:8px;line-height:1.4; }
      .bt-tl { display:flex;flex-direction:column;border-radius:16px;background:var(--bt-chip);padding:2px 14px;margin-bottom:12px; }
      .bt-tl-row { display:flex;align-items:baseline;gap:12px;padding:10px 0;border-top:1px solid var(--bt-line); }
      .bt-tl-row:first-child { border-top:none; }
      .bt-tl-time { width:52px;flex-shrink:0;font-size:14px;font-weight:600;font-variant-numeric:tabular-nums;color:var(--bt-ink2); }
      .bt-tl-label { flex:1;min-width:0;font-size:16px;font-weight:600; }
      .bt-tl-dur { font-size:13px;color:var(--bt-ink2);font-variant-numeric:tabular-nums; }
      .bt-note { font-size:13px;color:var(--bt-ink2);line-height:1.4;margin:0 0 6px; }
    `;
    ...
    hdr.innerHTML = `
      ...
      <button type="button" class="bt-close-btn" aria-label="Close">${CLOSE_SVG}</button>`;
    hdr.querySelector('.bt-close-btn').addEventListener('click', () => this._closePopup());
  ...
  _openPopup() {
    ...
    hero.innerHTML = `
      ...
      ${r.unit ? `<div style="font-size:16px;color:var(--bt-ink2);font-weight:500;">${esc(r.unit)}</div>` : ''}`;
    popup.appendChild(hero);
    const sub = document.createElement('div');
    sub.style.cssText = 'font-size:13px;color:var(--bt-ink2);margin-bottom:14px;';
    ...
    if (r.num !== null || binary) {
      ...
      const graph = document.createElement('div'); graph.className = 'bt-graph';
      [1, 3, 6, 12, 24].forEach(h => {
        const b = document.createElement('button');
        b.type = 'button'; b.className = 'bt-seg-btn' + (h === this._popupHours ? ' active' : ''); b.textContent = `${h}h`; b.dataset.hours = h;
        b.addEventListener('click', () => {
          this._popupHours = h;
          seg.querySelectorAll('.bt-seg-btn').forEach(x => x.classList.toggle('active', parseInt(x.dataset.hours, 10) === h));
    ...
    const rows = [['State', `${r.heroText}${r.unit ? ' ' + r.unit : ''}`], ['Last changed', timeAgo(r.stateObj.last_changed || r.stateObj.last_updated)]];
    rows.forEach(([l, v]) => {
      const row = document.createElement('div'); row.className = 'bt-info-row';
      row.innerHTML = `<span class="bt-info-label">${esc(l)}</span><span class="bt-info-value">${esc(v)}</span>`;
      info.appendChild(row);
    });
    popup.appendChild(info);

    // actions
    const btns = document.createElement('div'); btns.className = 'bt-btn-row';
    if (this._canAct()) {
      const t = document.createElement('button'); t.type = 'button'; t.className = 'bt-btn';
      t.textContent = this._actionLabel();
      t.addEventListener('click', () => { this._runAction(this._tapAction()); });
      btns.appendChild(t);
    }
  ...
  // The entity's area name (its own area, else its device's area), or ''
  _aiArea() {
    const id = this._stateId();
    const ent = this._hass?.entities?.[id];
    const areaId = ent?.area_id
      || (ent?.device_id && this._hass?.devices?.[ent.device_id]?.area_id);
  ...
  _aiContextText() {
    ...
    const id = this._stateId(), area = this._aiArea();
    const fn = r?.attrs?.friendly_name;
    return [
      `You are the assistant inside a smart-home dashboard card for a ${spec.appliance} named "${r?.name || 'this entity'}"${fn && fn !== r?.name ? ` (full name "${fn}")` : ''}, entity ID ${id}${area ? `, located in the ${area}` : ''}.`,
      `Discuss only this one entity. Ignore any other devices you know about, and never name a room unless one is given here.`,
      `Local time: ${clock}.`,
...
- status: one sentence (max 24 words) saying what the current state or reading means. Only use facts given above.
- next: one short sentence (max 16 words) telling the user what they could do themselves, or "" if there is nothing worth doing. Do not repeat the status.
You cannot control devices or take any action from this card, and the user cannot reply. Never offer to do something, never ask a question, and never write "Would you like me to…", "Shall I…" or "I can…".
Plain text only. No emojis.`;
    const raw = await this._aiConverse(prompt, { key: this._aiCacheKey('insight'), force });
    if (!raw) return null;
    const j = this._aiExtractJson(raw);
    if (j && typeof j.status === 'string') return { status: j.status.trim(), next: this._aiCleanNext(j.next) };
...
Answer in at most 45 words, plain text, no markdown or emojis. Be practical and cautious; if you don't know, say so.`;
  ...
  _aiSection(parent, label) {
    ...
    sec.className = 'bt-sec';
    if (label) { const l = document.createElement('div'); l.className = 'bt-sec-label'; l.textContent = label; sec.appendChild(l); }
  ...
  _aiSkeleton(el, lines = 2) {
    el.innerHTML = Array.from({ length: lines }, (_, i) => `<div class="bt-skel" style="width:${i === lines - 1 ? 62 : 100}%"></div>`).join('');
  }
  ...
  _openActionsSheet() {
    ...
    if (this._canAct()) rows.push({ icon: 'power', danger: false, label: this._actionLabel(), fn: () => this._runAction(this._tapAction()) });
    if (this._aiEnabled()) {
    ...
    list.className = 'bt-rows';
    rows.forEach(row => {
      const b = document.createElement('button');
      b.type = 'button'; b.className = 'bt-row' + (row.danger ? ' is-danger' : '');
  ...
  async _openInsightSheet() {
    ...
    now.body.innerHTML = `<div class="bt-local">${esc(this._aiSnapshot().local || '—')}</div>`;
    ...
    if (!alive()) return;
    if (!res) { assist.body.innerHTML = `<div class="bt-ai-text">I couldn’t reach the assistant. Check the AI settings in this card’s editor, then try again.</div>`; return; }
    assist.body.innerHTML = `<div class="bt-ai-text">${esc(res.status)}</div>`;
    if (res.next) { next.sec.hidden = false; next.body.innerHTML = `<div class="bt-ai-text">${esc(res.next)}</div>`; }
  }
  ...
  _openAskSheet() {
    ...
    if (!popup) return;
    const chips = document.createElement('div'); chips.className = 'bt-chips-q'; popup.appendChild(chips);
    const answer = document.createElement('div'); popup.appendChild(answer);
    const row = document.createElement('div'); row.className = 'bt-ask-row';
    row.innerHTML = `<input type="text" class="bt-ask-input" placeholder="Ask about this ${esc(spec.appliance)}…" autocomplete="off" enterkeyhint="send">
                     <button type="button" class="bt-send" aria-label="Send">${AI_ICONS.send}</button>`;
    ...
    const ask = async q => {
      ...
      answer.innerHTML = `<div class="bt-answer"><div class="bt-q-title">${esc(q)}</div><div class="bt-ans-body"></div></div>`;
      const bodyEl = answer.querySelector('.bt-ans-body');
    ...
    spec.defaultQuestions.forEach(q => {
      const b = document.createElement('button'); b.type = 'button'; b.className = 'bt-q'; b.textContent = q;
  ...
  async _openWeekSheet() {
    ...
    if (!alive()) return;
    const tile = (v, l) => `<div class="bt-stat"><b>${esc(v)}</b><span>${esc(l)}</span></div>`;
    const noHistory = `<div class="bt-ai-text">I couldn’t find any history for this ${esc(spec.appliance)} in the last 7 days. (Home Assistant’s recorder needs to keep this entity.)</div>`;
    const sum = document.createElement('div'); sum.className = 'bt-ai-text';
  ...
  async _openRecapSheet() {
    ...
    const fv = v => `${fmtNumber(v, dec)}${r.unit ? ' ' + r.unit : ''}`;
    const tile = (v, l) => `<div class="bt-stat"><b>${esc(v)}</b><span>${esc(l)}</span></div>`;
    const sum = document.createElement('div'); sum.className = 'bt-ai-text'; sum.style.marginTop = '10px';
    let prompt = '', key = '';

    if (spec.numeric) {
      const st = this._aiNumStats(series, start, end);
      if (!st) { body.innerHTML = `<div class="bt-ai-text">I couldn’t find enough history for this ${esc(spec.appliance)} in the last 24 hours.</div>`; return; }
      const change = r.num - st.first.v, sign = change > 0 ? '+' : '';
      const when = p => hhmm(p.t);
      body.innerHTML = `<div class="bt-tl-head">Last 24 hours</div>
        <div class="bt-stats">
          ${tile(fv(r.num), 'now')}
      ...
      if (!runs.length) { body.innerHTML = `<div class="bt-ai-text">I couldn’t find any recent activity for this ${esc(spec.appliance)} in the last 7 days. (Home Assistant’s recorder needs to keep this entity.)</div>`; return; }
      ...
      body.innerHTML = `<div class="bt-tl-head">${esc(head)}</div>
        <div class="bt-tl">${evs.map(e => `<div class="bt-tl-row"><span class="bt-tl-time">${esc(clockAt(e.t))}</span><span class="bt-tl-label">${esc(e.label)}</span><span class="bt-tl-dur">${e.dur != null ? esc(durText(e.dur)) : ''}</span></div>`).join('')}</div>
        ${notes.map(n => `<div class="bt-note">${esc(n)}</div>`).join('')}`;
      body.appendChild(sum);
  ...
  async _loadGraph(container, hours, r) {
    ...
    const msg = t => `<div class="bt-graph-msg">${t}</div>`;
    container.innerHTML = msg('Loading…');
    const end = new Date(), start = new Date(end - hours * 3600000);
    const color = tuneColor(isHex(this._config.active_color) ? this._config.active_color.trim() : DEFAULT_ACTIVE, this._dark).dot;
    try {
  ...
  _renderLine(container, pts, t0, t1, color, r) {
    ...
    const gid = 'btg' + Math.random().toString(36).slice(2, 7);
    container.innerHTML = `
      <svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" style="width:100%;height:100%;display:block;overflow:visible;touch-action:none;cursor:crosshair;">
        ...
        <line x1="0" y1="${Y(vmax).toFixed(1)}" x2="${W}" y2="${Y(vmax).toFixed(1)}" stroke="var(--bt-line)" stroke-dasharray="3 4" vector-effect="non-scaling-stroke"/>
        <line x1="0" y1="${Y(vmin).toFixed(1)}" x2="${W}" y2="${Y(vmin).toFixed(1)}" stroke="var(--bt-line)" stroke-dasharray="3 4" vector-effect="non-scaling-stroke"/>
      ...
      <div style="position:absolute;left:0;top:0;font-size:10px;font-weight:600;color:var(--bt-ink2);">${f(vmax)}${r.unit ? ' ' + esc(r.unit) : ''}</div>
      <div style="position:absolute;left:0;bottom:16px;font-size:10px;font-weight:600;color:var(--bt-ink2);">${f(vmin)}</div>
      <div style="position:absolute;left:0;right:0;bottom:0;display:flex;justify-content:space-between;font-size:10px;color:var(--bt-ink2);"><span>${hhmm(t0)}</span><span>${hhmm(t1)}</span></div>
      <div class="bt-xline" style="position:absolute;top:${(padT / H * 100).toFixed(2)}%;bottom:${(padB / H * 100).toFixed(2)}%;width:1px;background:${color};opacity:.7;display:none;pointer-events:none;"></div>
      <div class="bt-xdot" style="position:absolute;width:10px;height:10px;margin:-5px 0 0 -5px;border-radius:50%;background:${color};border:2px solid var(--bt-ink);box-sizing:border-box;display:none;pointer-events:none;"></div>
      <div class="bt-xpill" style="position:absolute;top:6px;min-width:54px;box-sizing:border-box;padding:6px 12px;border-radius:14px;text-align:center;pointer-events:none;display:none;border:0.75px solid var(--bt-chipedge);box-shadow:0 4px 14px rgba(0,0,0,0.35);-webkit-backdrop-filter:blur(14px);backdrop-filter:blur(14px);">
        <div class="bt-xpill-val" style="font-size:13px;font-weight:700;line-height:1.15;color:${color};white-space:nowrap;"></div>
        <div class="bt-xpill-time" style="font-size:10px;font-weight:500;color:var(--bt-ink2);margin-top:2px;white-space:nowrap;"></div>
      </div>`;

    const svg = container.querySelector('svg');
    const xline = container.querySelector('.bt-xline'), xdot = container.querySelector('.bt-xdot'), xpill = container.querySelector('.bt-xpill');
    const xval = xpill.querySelector('.bt-xpill-val'), xtime = xpill.querySelector('.bt-xpill-time');
    xpill.style.background = `linear-gradient(${hexA(color, 0.16)},${hexA(color, 0.16)}),var(--bt-chip)`;
  ...
  _renderSteps(container, pts, t0, t1, color, r) {
    ...
    container.innerHTML = `
      <div style="position:relative;">
        <svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" style="width:100%;height:${H}px;display:block;touch-action:none;cursor:crosshair;">
          <rect x="0" y="${padT}" width="${W}" height="${barH}" rx="10" fill="var(--bt-track)"/>
        ...
        <div class="bt-xline" style="position:absolute;top:0;bottom:0;width:1px;background:${color};opacity:.7;display:none;pointer-events:none;"></div>
        <div class="bt-xdot" style="position:absolute;top:${padT + barH / 2}px;width:10px;height:10px;margin:-5px 0 0 -5px;border-radius:50%;border:2px solid var(--bt-ink);box-sizing:border-box;display:none;pointer-events:none;"></div>
        <div class="bt-xpill" style="position:absolute;top:0;min-width:54px;box-sizing:border-box;padding:6px 12px;border-radius:14px;text-align:center;pointer-events:none;display:none;border:0.75px solid var(--bt-chipedge);box-shadow:0 4px 14px rgba(0,0,0,0.35);-webkit-backdrop-filter:blur(14px);backdrop-filter:blur(14px);">
          <div class="bt-xpill-val" style="font-size:13px;font-weight:700;line-height:1.15;white-space:nowrap;"></div>
          <div class="bt-xpill-time" style="font-size:10px;font-weight:500;color:var(--bt-ink2);margin-top:2px;white-space:nowrap;"></div>
        </div>
      </div>
      <div style="display:flex;justify-content:space-between;font-size:10px;color:var(--bt-ink2);margin-top:6px;"><span>${hhmm(t0)}</span><span>${hhmm(t1)}</span></div>
      <div style="margin-top:10px;font-size:13px;color:var(--bt-ink2);"><b style="color:var(--bt-ink);">On for ${dur(onMs)}</b> · ${Math.round(onMs / total * 100)}% of this period</div>`;
    ...
    const show = clientX => {
      ...
      xdot.style.background = xline.style.background = tint || 'var(--bt-ink2)';
      xpill.style.background = tint ? `linear-gradient(${hexA(tint, 0.16)},${hexA(tint, 0.16)}),var(--bt-chip)` : 'var(--bt-chip)';
      xval.textContent = labels[seg.on ? 0 : 1];
      xval.style.color = tint || 'var(--bt-ink2)';
      xtime.textContent = hhmm(t);
      const pw = xpill.offsetWidth;
...
const LAYOUT_OPTIONS = [
  { id: 'pill', name: 'Live Activity', sub: 'One row · icon, name, state', svg: _thumb(
      _glass(8, 22, 80, 20, 10) +
      `<circle cx="20" cy="32" r="6" fill="url(#lt-a)"/>` +
  ...
  { id: 'tile', name: 'Tile', sub: 'Compact · icon + name', svg: _thumb(
      ...
      `<circle cx="29" cy="32" r="7" fill="url(#lt-a)"/>` +
      `<rect x="41" y="26" width="26" height="4" rx="2" fill="rgba(255,255,255,0.85)"/><rect x="41" y="33" width="18" height="3" rx="1.5" fill="rgba(255,255,255,0.4)"/>`) },
  { id: 'square', name: 'Square', sub: 'Icon on top, name below', svg: _thumb(
      _glass(25, 9, 46, 46, 11) +
      `<circle cx="36" cy="20" r="6" fill="url(#lt-a)"/>` +
      `<rect x="31" y="39" width="26" height="4" rx="2" fill="rgba(255,255,255,0.85)"/><rect x="31" y="46" width="16" height="3" rx="1.5" fill="rgba(255,255,255,0.4)"/>`) },
];

class CrowButtonCardEditor extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: 'open' });
    this._config = {};
  ...
  _defaultIcon() {
    const so = this._hass?.states?.[this._config.state_entity || this._config.entity];
    return so ? defaultIconFor(so) : 'mdi:gesture-tap-button';
  }
  ...
  _render() {
      ...
      <div class="container">
        ...
        <div>
          ...
          <div class="card-block">
            <div class="select-row">
              <label>What the button controls</label>
              <div class="hint">A light, switch, script, scene, lock, cover, vacuum, button…</div>
              <div class="sel-host" id="ent-host"></div>
            </div>
            <div class="select-row">
              <label>Read its state from another entity <span class="badge-optional">Optional</span></label>
              <div class="hint">Useful when the state you want to show lives on a different entity — for example a script that starts the vacuum, showing the vacuum's state</div>
              <div class="sel-host" id="state-host"></div>
            </div>
            <div class="text-row">
              <label for="active_state">Counts as “on” when the state is <span class="badge-optional">Optional</span></label>
              <div class="hint">Separate several with commas — e.g. <i>cleaning, returning, paused</i>. Empty means the usual on / open / playing states.</div>
              <input type="text" id="active_state" placeholder="on">
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="card-block">
            <div class="toggle-list">
              ${tog('show_icon', 'Show icon', 'The icon is what you tap to switch the entity. Turned off, the whole button switches it instead.')}
            </div>
            <div id="icon_wrap">
              ...
              <div class="toggle-list" style="border-top:1px solid rgba(128,128,128,0.08);">
                ${tog('use_dynamic_icon', 'Use the entity\'s own icon', 'The icon follows the state (a lock locks, a cover opens) — ignores the icon chosen above')}
                ${tog('spin_icon', 'Spin while on', 'The icon spins smoothly for as long as the entity is on')}
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="card-block">
            ...
            <div class="toggle-list" style="border-top:1px solid rgba(128,128,128,0.08);">
              ${tog('show_state', 'Show state', 'Say “On”, “Off”, “Open”… under the name')}
            </div>
            ...
            ${slider('card_height', 'Button height', 'For the Live Activity layout — Tile and Square keep their proportions', 40, 110, 2)}
          </div>
        </div>
        ...
        <div>
          ...
          <div class="card-block">
            <div class="select-row">
              <label>When the icon is tapped</label>
              <div class="hint" id="tap_hint">Toggle switches the entity the usual way. For any other action, add a <code>tap_action</code> with <code>perform_action</code> in YAML.</div>
              <div class="seg" id="tap_seg">
                <button type="button" class="seg-btn" data-tap="toggle">Toggle</button>
                <button type="button" class="seg-btn" data-tap="none">Nothing</button>
                <button type="button" class="seg-btn" data-tap="custom" id="tap_custom" hidden>Custom (YAML)</button>
              </div>
            </div>
            <div class="select-row">
              <div class="hint">Tapping anywhere else on the card opens the details sheet — the current value, its history and when it last changed. A long-press opens the AI actions sheet when AI features are on.</div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="card-block">
            <div class="toggle-list">
              ${tog('flash_enabled', 'Flash while on', 'The glow pulses gently for as long as the entity is on. Turned off, it just stays lit.')}
            </div>
            <div id="flash_row" style="border-top:1px solid rgba(128,128,128,0.08);">
              ${slider('flash_speed', 'Flash speed', 'The time between the brightest and the dimmest moment (milliseconds) — lower is faster', 150, 1500, 50)}
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="card-block">
            ...
            <div class="select-row">
              ...
              <div class="hint">Off stops the flash too. System is Subtle but stays still if your iPhone's Reduce Motion is on.</div>
        ...
        <div>
          ...
          <div class="card-block">
            <div class="select-row">
              <label>Preset</label>
              <div class="hint">One tap sets the “on” colour — then adjust it or add the optional ones below</div>
            ...
            <div class="select-row">
              <div class="hint">The “on” colour is adjusted automatically so the button stays readable in both light and dark themes. Text and icon colours you choose are used exactly as picked.</div>
            </div>
          </div>
        </div>
    ...
    this._mountEntity('state-host', 'state_entity');
    this._mountIcon();
  ...
  _mountEntity(hostId, key) {
    ...
    if (customElements.get('ha-selector')) {
      ...
      el.hass = this._hass; el.selector = { entity: {} }; el.value = this._config[key] || ''; el.label = '';
  ...
  _syncUI() {
    ...
    ['entity', 'state_entity'].forEach(k => { const el = this._entSel?.[k]; if (el && el.value !== (cfg[k] || '')) el.value = cfg[k] || ''; });
    if (this._iconSel) {
    ...
    chk('spin_icon', cfg.spin_icon);
    chk('flash_enabled', cfg.flash_enabled !== false);
    set('name', cfg.name);
    set('active_state', Array.isArray(cfg.active_state) ? cfg.active_state.join(', ') : '');
    set('glass', Number.isFinite(parseFloat(cfg.glass)) ? parseFloat(cfg.glass) : 50);
    const fs = px(cfg.font_size, 14), ch = px(cfg.card_height, 56), sp = Math.min(1500, Math.max(150, parseInt(cfg.flash_speed, 10) || 600));
    set('font_size', fs); set('card_height', ch); set('flash_speed', sp);
    const lab = (id, t) => { const el = root.getElementById(id); if (el) el.textContent = t; };
    lab('font_size_v', `${fs}px`); lab('card_height_v', `${ch}px`); lab('flash_speed_v', `${sp} ms`);
    ...
    dim('flash_row', cfg.flash_enabled === false);
    ...
    // tap: Toggle / Nothing — or "Custom" when the YAML holds a service call
    const ta = cfg.tap_action && typeof cfg.tap_action === 'object' ? cfg.tap_action : (typeof cfg.tap_action === 'string' ? { action: cfg.tap_action } : null);
    const custom = !!(ta && ta.action !== 'toggle' && ta.action !== 'none' && (ta.perform_action || ta.service));
    const tapMode = custom ? 'custom' : (ta && ta.action === 'none' ? 'none' : 'toggle');
    root.getElementById('tap_custom').hidden = !custom;
    root.querySelectorAll('[data-tap]').forEach(b => b.classList.toggle('is-selected', b.dataset.tap === tapMode));
  ...
  _aiMarkup() {
    ...
    return `
        <div>
          ...
          <div class="card-block">
            <div class="toggle-list">
              ${tog('ai_features_enabled', 'Enable AI features', 'Long-press the button for an actions sheet: Insight, Ask AI, What happened? and This week.', on)}
            </div>
            <div id="ai_rows" style="${on ? '' : 'display:none'}">
              ...
              <div class="toggle-list" style="border-top:1px solid rgba(128,128,128,0.10);">
                ${tog('ai_enable_insight', 'Insight', 'What the current state means, with a tip and a next step', cfg.ai_enable_insight !== false)}
  ...
  _attachListeners() {
    ...
    root.querySelectorAll('[data-tap]').forEach(b => b.addEventListener('click', () => {
      if (b.dataset.tap === 'custom') return;
      this._set('tap_action', b.dataset.tap === 'none' ? { action: 'none' } : null);   // Toggle is the default, so nothing is written
    }));

    ['show_state', 'show_icon', 'use_dynamic_icon', 'spin_icon', 'flash_enabled'].forEach(id => $(id).addEventListener('change', e => this._set(id, e.target.checked)));
    $('name').addEventListener('input', e => this._set('name', e.target.value));
    $('active_state').addEventListener('change', e => {
      const list = e.target.value.split(/[,\n]/).map(s => s.trim()).filter(Boolean);
      this._set('active_state', list.length ? list : null);
    });
    ...
    $('flash_speed').addEventListener('input', e => this._set('flash_speed', parseInt(e.target.value, 10)));
  ...
  _dispatch() {
    ...
    Object.entries(CrowButtonCard.DEFAULTS).forEach(([k, v]) => { if (out[k] === v && k !== 'entity') delete out[k]; });
...
if (!customElements.get('crow-button-card')) customElements.define('crow-button-card', CrowButtonCard);
if (!customElements.get('crow-button-card-editor')) customElements.define('crow-button-card-editor', CrowButtonCardEditor);

window.customCards = window.customCards || [];
if (!window.customCards.some(c => c.type === 'crow-button-card')) {
  window.customCards.push({
    type: 'crow-button-card',
    name: 'Crow Button Card',
    preview: true,
    description: 'A liquid-glass entity button — pill, tile or square, with a flashing glow while on, your own Home Assistant icon, light and dark themes and your own colours.',
  });
}
